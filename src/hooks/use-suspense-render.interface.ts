 ...
 */
export type SuspenseRenderLoading<Data> =
  | React.ReactNode
  | ((data?: Promise<Data>) => React.ReactNode);

/**
 * When the async task is rejected, the error render function will be called.
 */
export type SuspenseRenderError<AsyncTaskError extends Error | unknown = unknown> =
  | React.ReactNode
  | ((error: AsyncTaskError) => React.ReactNode);
